...
public class TrialManager : MonoBehaviour {
    ...
    public StartingPointListener startingPointListener;
    public PlanePositionTransformer planePositionTransformer;
    ...
    public bool trialStarted {
        get {
            return this._trialStarted;
        }
        set {
            planePositionTransformer.nr_clutches = 0;
            this._trialStarted = value;
            if (value == false)
            {
                planeTransform.gameObject.GetComponent<BoxCollider>().enabled = false;
            } else {
                planeTransform.gameObject.GetComponent<BoxCollider>().enabled = true;
            }
        }
    }
    ...
    public void StartTrial() {
        ...
        notificationText.text = "Place finger in the middle of the screen";
        // create file
        ...
        // set target's local position;
        planeTransform.localPosition = Vector3.zero;
        targetTransform.localPosition = new Vector3(targetPositions[Convert.ToInt32(TrialSlider.value)].x_pos * Globals.SCALE, 0.0005f, targetPositions[Convert.ToInt32(TrialSlider.value)].y_pos * Globals.SCALE);

        // reset,prepare and wait for finger to be in the starting position
        startingPointListener.gameObject.SetActive(true);
        planePositionTransformer.nr_clutches = 0;
        planePositionTransformer.PreparePlaneTransformer();
    ...
    private IEnumerator RecordTrial()
    {
        while (true)
        {
            if (trialStarted == true)
            {
                ...
                //nr_of_clutches
                int nr_of_clutches = planePositionTransformer.nr_clutches;

                //is_panning
                int is_panning = planePositionTransformer.isPanning? 1 : 0;
    ...
    public void StopTrial() {
        transportServer.SendCommand("trialStatus|3|Trial Ended!");
        notificationText.text = "Trial Ended!";
        startingPointListener.gameObject.SetActive(false);
        planePositionTransformer.FinishPlaneTransformer();
