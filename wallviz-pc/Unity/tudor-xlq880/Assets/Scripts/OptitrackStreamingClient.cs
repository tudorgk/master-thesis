...
public class OptitrackOtherMarker
{
    public Vector3 Position;
}
...
public class OptitrackOtherMarkersState
{
    public OptitrackHiResTimer.Timestamp DeliveryTimestamp;
    public List<OptitrackOtherMarker> OtherMarkers;

    public override string ToString()
    {
        string returnString = "OtherMarkersState: Timesstamp = " + DeliveryTimestamp.ToString() + "\n" +
            "Count = " + OtherMarkers.Count + "\n";
        foreach (OptitrackOtherMarker marker in OtherMarkers) {
            returnString += "Marker: " + marker.Position.ToString() + "\n"; 
        }
        return returnString;
    }
}
...
public class OptitrackStreamingClient : MonoBehaviour
{
    ...
    /// <summary>Maps from a streamed other marker to its most recent available pose data.</summary>
    private OptitrackOtherMarkersState m_latestOtherMarkerState;
    ...
    public OptitrackOtherMarkersState GetLatestOtherMarkersState()
    {
        OptitrackOtherMarkersState omState;

        lock (m_frameDataUpdateLock)
        {
            omState = m_latestOtherMarkerState;
        }
    ...
    private void OnNatNetFrameReceived( object sender, NatNetClient.NativeFrameReceivedEventArgs eventArgs )
    {
        ...
        try
        {
            ...
            for ( int rbIdx = 0; rbIdx < frameRbCount; ++rbIdx )
            {
                ...
                bTrackedThisFrame = (rbData.Params & 0x01) != 0;
            ...
            if (bTrackedThisFrame) {          
                // Ensure we have a state of other markers
                OptitrackOtherMarkersState omState = GetOrCreateOtherMarkersState();
                omState.DeliveryTimestamp = OptitrackHiResTimer.Now();

                // Clear last markers
                omState.OtherMarkers.Clear();

                // Get other markers
                sOtherMarker[] otherMarkers = MarshalOtherMarkers(eventArgs.MarshaledFrame);
                for (int i = 0; i < eventArgs.MarshaledFrame.OtherMarkerCount; i++)
                {
                    sOtherMarker marker = otherMarkers[i];
                    // Change of basis from right- to left-handed by inverting X.
                    omState.OtherMarkers.Add(new OptitrackOtherMarker
                    {
    ...
    private OptitrackOtherMarkersState GetOrCreateOtherMarkersState()
    {
        if (m_latestOtherMarkerState == null)
        {
            m_latestOtherMarkerState = new OptitrackOtherMarkersState();
            m_latestOtherMarkerState.OtherMarkers = new List<OptitrackOtherMarker>();

        }

        return m_latestOtherMarkerState;
    }

    private sOtherMarker[] MarshalOtherMarkers(sFrameOfMocapData frameData)
    {
        sOtherMarker[] returnArray = new sOtherMarker[frameData.OtherMarkerCount];

        IntPtr buffer = Marshal.AllocCoTaskMem(Marshal.SizeOf(typeof(sOtherMarker)) * frameData.OtherMarkerCount);

        for (int i = 0; i < frameData.OtherMarkerCount; ++i)
        {
            returnArray[i] = (sOtherMarker)Marshal.PtrToStructure(frameData.OtherMarkers, typeof(sOtherMarker));
            frameData.OtherMarkers = new IntPtr(frameData.OtherMarkers.ToInt64() + Marshal.SizeOf(typeof(sOtherMarker)));
        }
